...
function getNeighbors(a, r, c, lowerOffset, upperOffset, includeSelf) {
  ...
  if (includeSelf) {
    ...
    const neighborGroups = arrayUtil.flatMap(_.zip(rowSlices, columnSlices), ([rowSlice, columnSlice]) => a.slice(rowSlice, columnSlice).tolist())
    const flat = arrayUtil.flatten(neighborGroups)
    return nj.array(flat) 
  }
}
