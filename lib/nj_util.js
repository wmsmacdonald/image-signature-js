...
function getNeighbors(a, r, c, lowerOffset, upperOffset, includeSelf) {
  ...
  if (includeSelf) {
    ...
    const neighborGroups = func.flatMap(_.zip(rowSlices, columnSlices), ([rowSlice, columnSlice]) => a.slice(rowSlice, columnSlice).tolist())
    const flat = [].concat.apply([], neighborGroups)
