...
function generate (imageData) {
  const img = nj.array(imageData.data).reshape(...imageData.shape)
  const gray = nj.images.rgb2gray(img)

  const cropped = autoCrop(gray)

  const gridAverages = computeGridAverages(cropped, 10, 10)

  const gridNeighbors = _.map(gridAverages.flatten(), (avg, idx) => getNeighbors(gridAverages, ...mCoords(idx, ...gridAverages.shape), 1, 1, false))

  const differentialGroups = _.map(_.zip(gridAverages, gridNeighbors), ([avg, neighbors]) => _.map(neighbors, neighbor => neighbor - avg))
