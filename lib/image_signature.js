...
function generate (imageData, numChannels) {
  numChannels = numChannels === undefined ? 4 : numChannels

  const img = nj.array(imageData.data)
  const rgb = img.reshape(imageData.height, imageData.width, numChannels)

  const gray = nj.images.rgb2gray(rgb)
