...
function computeGridSquares(imageArray, numBlocksHigh, numBlocksWide) {
  ...
  const gridYs = _.map(_.range(squareHeight, imageArray.shape[0]), Math.floor)
  const gridXs = _.map(_.range(squareWidth, imageArray.shape[1]), Math.floor)
  console.log('gridYs', gridYs)
  ...
  const neighborGroupsFlat = _.map(gridCoords, ([x,y]) => {
    // TODO use P rather than static 3x3
    const rowSlice = [roundOffRow(x - 1), roundOffRow(x + 1) + 1]
    const columnSlice = [roundOffColumn(y - 1), roundOffColumn(y + 1) + 1]
    const neighbors = imageArray.slice(rowSlice, columnSlice).flatten().tolist()
    return neighbors.concat(_.range(0, 9 - neighbors.length).fill(0))
  })
  ...
  return array.reshape(numBlocksHigh - 1, numBlocksWide - 1, 9)
}
